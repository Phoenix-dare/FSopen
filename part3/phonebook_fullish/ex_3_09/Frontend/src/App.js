    ...
    if (doesExist) {
      ...
      if (window.confirm(`${newName} is already added to phonebook.Do you want to replace old number with the new one`) && newNumber) {
        ...
        personService.update(doesExist.id, { ...added, number: newNumber })
          ...
          .catch(error => {
            console.log(error)
            setErrorStatus(true)
            setMessage(`Something went wrong while updating ${doesExist.name}`)
            setTimeout(() => {
              setMessage(null);
            }, 4000)
          })
      ...
      personService.create(added)
        .then(data => {
          ...
          setNewName('')
          setNewNumber('')
          setMessage(`Added contact "${added.name}" successfully`)
          setErrorStatus(false)
        ...
        .catch(error => {
          console.log(error)
          setErrorStatus(true)
          setMessage(`Something went wrong while adding ${added.name}`)
          setTimeout(() => {
            setMessage(null);
          }, 4000)
        })
  ...
  const deleteUser = (id, name) => {
    if (window.confirm(`Are you sure you want to remove ${name} from contacts`)) {
      personService
        ...
        .catch(error => {
          setErrorStatus(true)
          setMessage(`Something went wrong while removing ${name} :${error}`)
          setTimeout(() => {
            setMessage(null);
          }, 4000)
        })
